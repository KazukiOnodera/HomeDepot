...
#items = pd.read_csv(hd.path_sub+'05item_tbl_query_selected.csv')['Unnamed: 0'].to_frame()
#items.columns = ['terms']
#items['order'] = 0
#items = items.terms.tolist()

items = pd.read_csv(hd.path_sub+'06sort_items_query.csv',index_col=0)['0'].tolist()
ng = pd.read_csv(hd.path_sub+'06sort_items_query_ng.csv',index_col=0)['0'].tolist()
...
# 
#==============================================================================
base = base[base.s_.map(lambda x:is_inc2(items,x))][base.r.isin(rr)]

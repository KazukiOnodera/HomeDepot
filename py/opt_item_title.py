...
items = pd.read_csv(ps+'item_tbl_selected_title.csv')['Unnamed: 0'].to_frame()
items.columns = ['terms']
items['order'] = 0

#items = pd.read_csv(ps+'items_opted_4328_119.csv',index_col=0)
...
class Gene:
    ...
    def get_f(self):
        ...
        base['item'] = base.t_.map(lambda x:get_item_list_in_list(items.terms.tolist(),x))
